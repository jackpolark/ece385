...
// -------------------------------- Score indices ------------------------------------------------
    parameter integer NUM_SIZE = 7;
    parameter integer NUM_OFFSET_X = 2;
    parameter integer NUM_OFFSET_Y = 2;
    parameter integer NUM_X_START = 132;
    
    parameter integer GR_Y_START = 129;
    parameter integer W_Y_START = 145;
    
    parameter integer SCOREBLOCK_START_X = 240;
    parameter integer SCOREBLOCK_START_Y = 208;
    parameter integer SCOREBLOCK_SIZE = 10;
    
    logic [9:0] numbers_location;
    logic [2:0] score_digits [5:0]; // hold up to 6 digits

    
    logic score_on;
    
    always_comb begin
        score_on = (DrawX >= NUM_X_START && DrawX < SCOREBLOCK_START_X + (6 * SCOREBLOCK_SIZE) 
            && DrawY >= SCOREBLOCK_START_Y && DrawY < SCOREBLOCK_START_Y + SCOREBLOCK_SIZE);
    end
 ...
 if(dog_frame_timer >= DOG_FRAME_DELAY) begin
    ...
    if(dog_state == DOG_WALKING || dog_state == DOG_SNIFFING || dog_state == DOG_ALERT 
        ...
        if(dog_state == DOG_WALKING) begin
            if(dog_frame_idx == 3'd3) begin
                dog_frame_idx <= dog_frame_idx - 1;
            end
            else if (dog_frame_idx == 3'd0) begin
                dog_frame_idx <= dog_frame_idx + 1;
            end
        end
...
// ----------------------------------------- Dog address indexing -----------------------------------------
    ...
    assign dog_x = dog_x_offset + dog_pixel_x;
...
//    duck_x_end   = '{297, 317, 337, 357, 377, 398, 418, 438, 458, 478};
    ...
    for (int i = 0; i < 10; i++) begin
        if (DrawY >= 415 && DrawY <= 433 && DrawX >= duck_x_start[i] && DrawX <= duck_x_end[i]) begin
            ...
            if (duck_counter == i && duck_hit_count > i) score_duck_update = 1;
        end
    end   
    ...
    if (flash_counter == 2'd3) begin
        ...
        else if (dog_on) begin
            if (dog_on_screen) begin
                Red = dog_red; Green = dog_green; Blue = dog_blue;
            end else begin
                Red = 4'hF; Green = 4'hA; Blue = 4'hC;
            end
        end else begin
            Red = gamea_red; Green = gamea_green; Blue = gamea_blue;
        end
        ...
        Red = 4'h7; Green = 4'h3; Blue = 4'h0;
    end
    else if (flyaway_notice_on && flyaway && rom_ducks != TRANSPARENT_COLOR) begin

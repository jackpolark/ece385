 ...
 WORD xfrlen = 0;
    ...
    while( 1 ) { // use a 'return' to exit this loop
        ...
        if ((MAXreg_rd(rHIRQ) & bmRCVDAVIRQ) == 0) {
            // If polling a HID interrupt-in endpoint, and toggle is correct,
            // then assume data could be valid (some mice fail to assert this)
            // So: wait a few cycles and retry instead of returning error
            ...
            // Still nothing? Soft-fail (return hrNAK instead of 0xf0)
            if ((MAXreg_rd(rHIRQ) & bmRCVDAVIRQ) == 0) {
                return (hrNAK); // act like no data instead of error
            }
        }

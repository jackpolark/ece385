...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int Status;
		    BYTE WriteBuffer[2] = { reg + 2, val }; // Command byte + data
		    BYTE ReadBuffer[2] = { 0 }; // Read buffer (not used but required for full-duplex)
	//psuedocode:
	//select MAX3421E 
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	    ...
	    // Perform SPI transfer: write command and value
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status;
	    ...
	    BYTE ReadBuffer[nbytes + 1]; // Unused but required for SPI full-duplex

	    WriteBuffer[0] = reg + 2; // Command byte
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int Status;
	    BYTE WriteBuffer[2] = { reg, 0 }; // Command byte, followed by a dummy byte
	    BYTE ReadBuffer[2] = { 0 }; // Response buffer
	...
	// Perform SPI transfer: send command, receive status + data
	    Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, sizeof(WriteBuffer));
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXreg_rd\n");
	    }

	    // Deselect MAX3421E
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00);

	    return ReadBuffer[1]; // Return the actual register value
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status;
	    ...
	    WriteBuffer[0] = reg; // Command byte
	    for (int i = 1; i <= nbytes; i++) {
	        WriteBuffer[i] = 0; // Dummy bytes for full-duplex
	    }
	...
	// Perform SPI transfer
	    Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes + 1);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXbytes_rd\n");
	    }

	    // Deselect MAX3421E
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00);

	    // Copy the received data (skip the first byte since it's the status)
	    for (int i = 0; i < nbytes; i++) {

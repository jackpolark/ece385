...
BYTE mousePoll(BOOT_MOUSE_REPORT* buf) {
    ...
    // Set the target peripheral address
    MAXreg_wr(rPERADDR, hid_device.addr);

    // Every N polls, force the data toggle to reset to DATA0
    if (++poll_count >= 5) {  // about every ~100ms depending on USB task rate
        devtable[hid_device.addr].epinfo[1].rcvToggle = bmRCVTOG0;
        MAXreg_wr(rHCTL, bmRCVTOG0);  // apply the reset toggle immediately
        poll_count = 0;
    }

    // Perform the actual IN transfer

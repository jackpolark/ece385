...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	//fill in this function to set the color palette starting at offset 0x0000 2000 (from base)
	 // 0x2000 byte offset = 0x800 word offset (because each word is 4 bytes)
	    volatile uint32_t* palette = ((volatile uint32_t*)hdmi_ctrl) + 0x800;

	    // RGB 12-bit encoding: R[11:8] G[7:4] B[3:0]
	    palette[color] = ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);
}

...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	// Construct the 16-bit color value: [13:8]=Red, [7:4]=Green, [3:0]=Blue
	    uint16_t color16 = ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);

	    // Each color is stored in its own 32-bit register at base + 0x2000 + 4*color
	    volatile uint32_t* palette_base = (volatile uint32_t*) hdmi_ctrl;
	    palette_base[0x800 + color] = (uint32_t) color16;
}

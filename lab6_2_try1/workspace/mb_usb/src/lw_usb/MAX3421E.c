...
void MAXreg_wr(BYTE reg, BYTE val) {
	//psuedocode:
	//select MAX3421E 
	XSpi_SetSlaveSelect(&SpiInstance, 0x01); //change 0x01 to proper u32 SlaveMask
	//write reg + 2 via SPI
	...
	// DONT USE BELOW CODE JUST FOR INITIAL TEST AND HINTS
	int Status;
	// Prepare buffer to send: reg + 2 (address offset) and val (value to write)
	    BYTE buffer[2] = { reg + 2, val };

	    // Perform SPI transfer: send register address and value
	    Status = XSpi_Transfer(&SpiInstance, buffer, NULL, sizeof(buffer));
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXreg_wr\n");
	    }

	    // Deselect MAX3421E after the transaction
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00); // Ensure the correct Slave Mask is used
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x01); //change 0x01 to actual u32 SlaveMask
	//write reg + 2 via SPI
	...
	// DONT USE BELOW CODE JUST FOR INITIAL TEST AND HINTS
	 int Status;
	 // Prepare the buffer: reg + 2 (address offset), followed by the data bytes
	    BYTE buffer[nbytes + 1];
	    buffer[0] = reg + 2; // Register address with offset
	    ...
	    // Perform SPI transfer: send register address and data bytes
	    Status = XSpi_Transfer(&SpiInstance, buffer, NULL, nbytes + 1);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXbytes_wr\n");
	    }

	    // Deselect MAX3421E after the transaction
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00); // Ensure the correct Slave Mask is used

	    // Return the pointer to the end of the data written (data + nbytes)
	    return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	 BYTE recvBuffer[1];  // Buffer to store received data CAN REMOVE
	//select MAX3421E (
	...
	// DONT USE BELOW CODE JUST FOR INITIAL TEST AND HINTS
	// Send the register address (reg + 2 for MAX3421E)
	int Status;
	 // Send register address (reg + 2 for MAX3421E)
	    Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXreg_rd (write)\n");
	    }

	    // Read the value from the register
	    Status = XSpi_Transfer(&SpiInstance, NULL, recvBuffer, 1);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXreg_rd (read)\n");
	    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status;
	 // Send register address (reg + 2 for MAX3421E)
	    Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXbytes_rd (write)\n");
	    }

	    // Read data bytes from SPI into the buffer
	    Status = XSpi_Transfer(&SpiInstance, NULL, data, nbytes);
	    if (Status != XST_SUCCESS) {
	        xil_printf("Error in SPI transfer during MAXbytes_rd (read)\n");
	    }

	    // Deselect MAX3421E after the transaction
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00); // Ensure the correct Slave Mask is used

	    // Return the pointer to the end of the data read (data + nbytes)
	    return (data + nbytes);
